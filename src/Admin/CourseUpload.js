...
const CourseUpload = ({ onClose, course }) => {
  ...
  useEffect(() => {
    ...
    if (course) {
      ...
      setSelectedInstructor(course.instructor.first_name);
      setSelectedCategory(course.category.title);
  ...
  // Create the initial courseData object without category and instructor_id
  const courseData = {
    title: courseTitle,
  ...
  // Conditionally add category and instructor_id only if selected
  if (selectedCategory) {
    courseData.category = parseInt(selectedCategory);
  }
  if (selectedInstructor) {
    courseData.instructor_id = parseInt(selectedInstructor);
  }
    ...
    try {
      ...
      setTimeout(() => navigate('a-courses'), 1000);
    } catch (error) {
